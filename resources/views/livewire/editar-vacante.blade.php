<form wire:submit.prevent="editarVacante" class="md:w-1/2 space-y-5 p-12 text-gray-900" enctype="multipart/form-data">
    <div>
    ...
    <div>
        @if (!$imagen)
            ...
            <x-text-input wire:model="imagen_nueva" id="imagen_nueva" type="file" accept="image/*" class="block mt-1 w-full" />
        ...
        @if ($imagen_nueva)
            <div class="w-80 my-5">
                ...
                <img src="{{ $imagen_nueva->temporaryUrl() }}" alt="Imagen cargada del usuario">
            </div>
        ...
        @if ($imagen)
            <div class="w-80 my-5">
                <p class="block font-medium text-lg text-gray-700 pl-2">Imagen cargada:</p>
                <img src="{{ asset('storage/vacantes/'.$imagen) }}" alt="Imagen cargada del usuario ({{ $titulo }})">
                <p class="block font-medium text-lg text-red-600 pt-2 pl-2 underline cursor-pointer" wire:click="eliminarImagen">Eliminar imagen</p>
            </div>
        @endif
